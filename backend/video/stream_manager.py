...
class StreamManager:
    def __init__(self):
        ...
        self.hls_dir = os.environ.get("HLS_OUTPUT_DIR", "/tmp/gvista-hls")
    ...
    async def start_stream(self, camera_id: str, rtsp_url: str) -> bool:
        ...
        # Initialize restart count if new
        ...
        # Setup callback for unexpected exits to handle auto-reconnect
    ...
    async def stop_stream(self, camera_id: str):
        ...
        # Clear callback so it doesn't trigger auto-reconnect on manual stop
    ...
    async def _handle_unexpected_exit(self, camera_id: str, rtsp_url: str):
        """Handles auto-reconnection logic when FFmpeg crashes."""
