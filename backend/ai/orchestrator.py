...
from .schemas import (
    AIProfile, FrameQuality, PlateStatus, PlateResult, AIAnalysisResult
)
from .config import (
    AI_PROFILES_CONFIG, OCR_CONFIDENCE_THRESHOLD, PLATE_CONFIDENCE_THRESHOLD
)
from .quality import FrameQualityAnalyzer
from .mock_providers import (
    MockVehicleDetector, MockPersonDetector, MockAnomalyDetector, MockPlateDetector, MockOCRProvider
)
    ...
    def analyze_frame(self, norm_frame: NormalizedFrame) -> AIAnalysisResult:
        ...
        if self.config.get("vehicle_detection"):
            ...
            if self.config.get("plate_detection"):
                for v in vehicles:
                    # In a real impl, crop using v.bbox. We just mock it.
